...
class ApiAsker < Asker
  ...
  validates :v_allocation_type, inclusion: { in: %w(ARE_ASP ASS_AER_APS_ASFNE RPS_RFPA_RFF_PENSION RSA AAH none), message: "%{value} n'est pas une valeur parmis celles possibles (ARE_ASP, ASS_AER_APS_ASFNE, RPS_RFPA_RFF_PENSION, RSA, AAH, none)" } , unless: Proc.new { |a| a.v_allocation_type.blank? }
