...
describe AidCalculationService do
  ...
  describe "Instantiation" do
    before do
      ...
      allow(aidtree_layer).to receive(:go).and_return(get_possible_aidtree)
    ...
    it 'Calcul must occur on instantiation' do
      ...
      #then
      expect(sut._all_aids).to eq(get_possible_aidtree)
    end
    ...
    it 'Calcul must augment against the given asker all_eligible' do
      # given
      sut  = AidCalculationService.get_instance(the_asker)
      # when
      sut.all_eligible
