...
class DetailWhy < ViewObject
  ...
  def after_init(args)
    ...
    @child_rules    = @ability_tree[:slave_rules]
  end
  ...
  def slave_rules
    if @ability == 'eligible'
      return @child_rules.sort_by{|rule| rule[:ability] == 'eligible' ? 0 : 1}
    elsif @ability == 'ineligible'
      return @child_rules.sort_by{|rule| rule[:ability] == 'ineligible' ? 0 : 1}
    elsif @ability == 'uncertain'
      return @child_rules.sort_by{|rule| rule[:ability] == 'uncertain' ? 0 : 1}
    end
    return []
  end
  ...
  def uncertain_sentence
    ...
    if @ability == 'uncertain'
      ...
      number_of_rules = @child_rules.length
  ...
  def number_of_uncertain_rules
    @child_rules.select{|rule| rule[:ability] == 'uncertain'}.length
  end

  def number_of_eligible_rules
    @child_rules.select{|rule| rule[:ability] == 'eligible'}.length
  end

  def all_conditions
    @child_rules.size > 1 && @child_rules.all? {|e| e[:ability] == 'eligible'}
  end

  def no_condition
    @child_rules.size > 1 && @child_rules.all? {|e| e[:ability] == 'ineligible'}
  end
