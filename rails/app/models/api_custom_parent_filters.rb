...
class ApiCustomParentFilters < ActiveType::Object
  ...
  def single_value_must_be_amongst_activated_filters
    if !filters.include?(",") && !has_filter(filters)
      errors.add(:custom_parent_filters, "La valeur doit être le slug d'un filtre parent personnalisé actif. Pour spécifier plusieurs slugs, il faut les séparer par une virgule.")
    end
  end
  ...
  def one_of_the_values_must_be_amongst_activated_filters
    if filters.include?(",")
      filters.split(",").each do |e|
        if !has_filter(e)
          errors.add(:custom_parent_filters, "#{e} n'est pas le slug d'un filtre parent personnalisé actif")
        end
      end
    end
  end
