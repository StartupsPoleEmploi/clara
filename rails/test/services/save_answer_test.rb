...
class SaveAnswerTest < ActiveSupport::TestCase
  ...
  test 'By default, saves an asker an modify session' do
    ...
    allow_any_instance_of(AnswerActivation).to receive(:get_value).and_return(true)
    #when
  ...
  test '.call, do not touch anything if AnswerActivation is set to false' do
    #given
    session = {}
    asker = Asker.new({v_age: '42'})
    before_answer_count = Answer.count
    allow_any_instance_of(AnswerActivation).to receive(:get_value).and_return(false)
    #when
    res = SaveAnswer.new.call(asker, session)
    #then
    after_answer_count = Answer.count
