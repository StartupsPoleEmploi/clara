class ResultDefault < ViewObject
  ...
  def icon_for(aids_per_contract)
    ...
    icon.blank? ? _default_svg.html_safe : icon.insert(icon.index("<svg") + 4, " width=\"50\" ").html_safe
  end
  ...
  def link_to_aid_detail(aid)
    for_id_param = @context.params["for_id"]
    "#{detail_path(aid["slug"])}?for_id=#{for_id_param}"
  end
