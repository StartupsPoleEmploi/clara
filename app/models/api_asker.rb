...
class ApiAsker < Asker
  ...
  validates :v_disabled, inclusion: { in: %w(true false), message: "%{value} n'est pas une valeur parmis celles possibles (true, false)" } , unless: Proc.new { |a| a.v_handicap.blank? }
  ...
  validates :v_duree_d_inscription, inclusion: { in: %w(categories_12345 other_categories), message: "%{value} n'est pas une valeur parmis celles possibles (more_than_a_year, less_than_a_year, not_registered)" } , unless: Proc.new { |a| a.v_duree_d_inscription.blank? }
  validates :v_duree_d_inscription, inclusion: { in: %w(categories_12345 other_categories), message: "%{value} n'est pas une valeur parmis celles possibles (more_than_a_year, less_than_a_year, not_registered)" } , unless: Proc.new { |a| a.v_duree_d_inscription.blank? }
